...
play_width = 600  # play window width; 600/20 = 20 width per block
play_height = 600  # play window height; 600/20 = 20 height per block
block_size = 30  # size of block
fps = 60
menu_state = 1 #Menu does not start on the screen (tracks number of entranced to the menu by each user)
...
#TESTED 
def draw_menu_window(surface, grid, menu_open):
    surface.fill((0, 0, 0))  # fill the surface with black
    #menu_button = Button(0, 0, 40, 40, "Menu", (255,255,255), (0,0,255), display_instructions)
    ...
    game_key = font.render('Left Shift - Menu', 1, (255, 255, 255))
    ...
    if menu_open:
        image = pygame.image.load("Menu.png").convert()
...
#For internal instructions they will have to be designed into this here (use the score thing as a guide)
#For external button, we will have to design a menu constructor that overlays the rest of the program
def main(window):
    locked_positions = {}
    ...
    max_game_duration = 6000 # 1 minute for testing, ten for trials
    elapsed_time = 0
    menu_access = 0
    menu_open = False
    ...
    while run:
        ...
        if menu_open:  
            draw_menu_window(window, grid, menu_open)
        else: 
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if not name_entered:
                    ...
                    print('external trial: ', input_text)
                    main(window)
